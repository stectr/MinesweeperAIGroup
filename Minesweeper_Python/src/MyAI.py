...
class MyAI( AI ):
	def __init__(self, rowDimension, colDimension, totalMines, startX, startY):
		...
		self.prob = [[(self.mineCount / self.rows * self.cols) for y in range(self.cols)] for x in range (self.rows)]
		self.uncovered = set()
	...
	def getAction(self, number: int) -> "Action Object":
		...
		if self.next_moves:
			...
			min_indices = [(x,y) for x,y in self.next_moves if self.prob[x][y] == min_value]
	...
	def calc_prob(self, x, y):
		...
		non_flagged_neighbors = [(nx, ny) for (nx, ny) in self.neighbors(x, y) if (nx, ny) not in self.uncovered and (nx, ny) not in self.flagged]
		...
		for (nx, ny) in self.explored_neighbors(x, y):
			...
			if hint > 0:
				...
				flagged_count = sum(1 for (tx, ty) in total_neighbors if (tx, ty) in self.flagged)
				covered_count = sum(1 for (tx, ty) in total_neighbors if (tx, ty) not in self.uncovered and (tx, ty) not in self.flagged)
