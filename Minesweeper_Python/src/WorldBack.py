...
class World():
	...
	def run(self) -> int:
		...
		if self.__score == (self.__colDimension * self.__rowDimension) - self.__totalMines:
			if self.__rowDimension == 5 and self.__colDimension == 5:
				return 1
			elif self.__rowDimension == 10 and self.__colDimension == 10:
				return 2
			elif self.__rowDimension == 10 and self.__colDimension == 18:
				return 3
			else:
				return 1
		else:
	...
	def __createBoard(self, inputStream: "filePointer" = None) -> None:
		...
		if inputStream:
			...
			self.__colDimension = 5		# Default sizes
			self.__rowDimension = 5		# Default size
	...
	def __addMines(self, inputStream: "filePointer" = None) -> None:
		...
		if inputStream:
			...
			currentMines = 0
			while currentMines < 1:	# Default number of mines is 10
